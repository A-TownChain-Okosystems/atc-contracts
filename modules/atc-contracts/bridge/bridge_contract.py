...
class BridgeContract:
    ...
    def get_tx(self, tx_id: str) -> BridgeTx | None:
        return self._txs.get(tx_id)

    def list_txs(self, addr: str | None = None) -> list[BridgeTx]:
    ...
    def pause(self, by: str):
        if by != self.owner:
            raise PermissionError("Nur Owner kann pausieren")
        self._paused = True

    def resume(self, by: str):
        if by != self.owner:
            raise PermissionError("Nur Owner kann fortsetzen")
        self._paused = False
