...
class GovernanceContract(BaseContract):
    ...
    def create_proposal(
        ...
        pid = "PROP-" + hashlib.sha256(f"{creator}{title}{now}".encode()).hexdigest()[:12].upper()
    ...
    def vote(self, voter: str, proposal_id: str, option: int) -> dict:
        ...
        proposal.vote_counts[str(option)] = proposal.vote_counts.get(str(option), 0.0) + weight
    ...
    def execute_proposal(self, caller: str, proposal_id: str) -> dict:
        ...
        if now < proposal.timelock_end:
            ...
            raise ValueError(
                f"Timelock aktiv: noch {remaining // 3600}h {(remaining % 3600) // 60}min"
            )
